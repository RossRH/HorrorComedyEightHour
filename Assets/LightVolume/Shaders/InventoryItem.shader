Shader "Sprites/InventoryItem"
{
    Properties
    {
        ...
        [MaterialToggle] _OutlineInside("Enable Outline", float) = 0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            VertexOutput ComputeVertex(VertexInput vertexInput)
            {
                ...
                vertexOutput.Color = vertexInput.Color * _RendererColor;
            ...
            int ShouldDrawOutlineInside2 (fixed4 sampledColor, float2 texCoord, int isOutlineEnabled, int outlineSize, float alphaThreshold)
            {
                // Won't draw if effect is disabled, outline size is zero or sampled fragment is opaque.
                if (isOutlineEnabled * outlineSize * sampledColor.a >= 0.01) return 0;
                ...
                for (int i = 1; i <= SAMPLE_DEPTH_LIMIT; i++)
                {
                    ...
                    if (pixelDownAlpha >= 0.99) return 1;
                    ...
                    if (pixelRightAlpha >= 0.99) return 1;
                    ...
                    if (pixelLeftAlpha >= 0.99) return 1;
            ...
            fixed4 ComputeFragment(VertexOutput vertexOutput) : SV_Target
            {
                ...
                int shouldDrawOutline = ShouldDrawOutlineInside2(color, vertexOutput.TexCoord, isOutlineEnabled, outlineSize, alphaThreshold);

                color.rgba = lerp(color.rgba, _OutlineColor.rgba, shouldDrawOutline);
